...
def getOverviewPlots(df, readmission_type, med_cols):
    ...
    chart = (alt.Chart(heatmap_df).
        ...
        size=alt.condition(alt.datum.count > 1, alt.Size('count:Q', scale=alt.Scale(type='sqrt', range=[150, 1000])),
                           alt.value(0)),  # TODO: decide whether linear or sqrt looks better
        # size=alt.condition(alt.datum.count > 1, alt.Size('count:Q', scale=alt.Scale(type='linear', range=[150, 1000])), alt.value(0)),
        color=alt.Color('percent:Q', title='Readmission rate (%)',
                        scale=alt.Scale(scheme='darkred', reverse=True, domain=[0, 100])),
